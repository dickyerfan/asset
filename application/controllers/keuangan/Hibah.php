...
class Hibah extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['title'] = 'RINCIAN MODAL HIBAH ' . $tahun;
        $data['hibah'] = $this->Model_keuangan->get_hibah($tahun);

        if ($this->session->userdata('bagian') == 'Keuangan') {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('keuangan/view_hibah', $data);
    ...
    public function cetak_hibah()
    {
        ...
        $this->pdf->filename = "hibah-{$tahun}.pdf";
        $this->pdf->generate('keuangan/cetak_hibah_pdf', $data);
    }

    public function input_hibah()
    {
        ...
        if ($this->form_validation->run() == false) {
            $data['title'] = 'INPUT HIBAH MODAL';
            $data['kec'] = $this->Model_keuangan->get_kec();
            $this->load->view('templates/header', $data);
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('keuangan/view_input_hibah', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $query = $this->db->get('ek_hibah');

            if ($query->num_rows() > 0) {
                $this->session->set_flashdata(
                    'info',
                    '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <strong>Gagal!</strong> Daftar Modal Hibah sudah ada.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>'
                );
                redirect('keuangan/hibah');
                return false;
            } else {
                $this->Model_keuangan->input_hibah('ek_hibah', $data_hibah);
                $this->session->set_flashdata(
                    'info',
                    '<div class="alert alert-primary alert-dismissible fade show" role="alert">
                        <strong>Sukses!</strong> Daftar Modal Hibah baru berhasil ditambahkan.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>'
                );
                redirect('keuangan/hibah');
    ...
    public function edit_hibah($id_hibah)
    {
        $tahun = $this->session->userdata('tahun_session');
        date_default_timezone_set('Asia/Jakarta');
        $data['title'] = 'Edit hibah PDAM';
        $data['hibah'] = $this->Model_keuangan->get_id_hibah($id_hibah);

        $this->load->view('templates/header', $data);
        $this->load->view('templates/navbar');
        $this->load->view('templates/sidebar');
        $this->load->view('keuangan/view_edit_hibah', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function update_hibah()
    {
        ...
        $id_hibah = $this->input->post('id_hibah');
        $keterangan = $this->input->post('keterangan');
        ...
        $this->Model_keuangan->update_hibah($id_hibah, $data_tka);
        $this->session->set_flashdata(
            'info',
            '<div class="alert alert-primary alert-dismissible fade show" role="alert">
        ...
        redirect('keuangan/hibah?tahun=' . $tahun);
    }
}
