...
class Modal_pemda extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['modal_pemda'] = $this->Model_keuangan->get_modal_pemda($tahun);

        if ($this->session->userdata('bagian') == 'Keuangan') {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('keuangan/view_modal_pemda', $data);
    ...
    public function cetak_modal_pemda()
    {
        ...
        $this->pdf->filename = "modal_pemda-{$tahun}.pdf";
        $this->pdf->generate('keuangan/cetak_modal_pemda_pdf', $data);
    }

    public function input_modal_pemda()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/navbar');
            $this->load->view('templates/sidebar');
            $this->load->view('keuangan/view_input_modal_pemda', $data);
            $this->load->view('templates/footer');
        } else {
            ...
            $query = $this->db->get('ek_modal_pemda');

            if ($query->num_rows() > 0) {
                $this->session->set_flashdata(
                    'info',
                    '<div class="alert alert-danger alert-dismissible fade show" role="alert">
                        <strong>Gagal!</strong> Daftar Modal Penyertaan Pemda sudah ada.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>'
                );
                redirect('keuangan/modal_pemda');
                return false;
            } else {
                $this->Model_keuangan->input_modal_pemda('ek_modal_pemda', $data_modal_pemda);
                $this->session->set_flashdata(
                    'info',
                    '<div class="alert alert-primary alert-dismissible fade show" role="alert">
                        <strong>Sukses!</strong> Daftar Modal Penyertaan Pemda baru berhasil ditambahkan.
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>'
                );
                redirect('keuangan/modal_pemda');
